...
class VERLPolicyWrapper(nn.Module):
    ...
    def generate(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            output = self.model(
                input_ids=generated_ids, attention_mask=full_attention_mask
            )
        ...
        log_probs, _ = self._compute_log_probs_tensor(
            generated_ids, scores, prompt_lengths
        )
    ...
    def _compute_log_probs(
        ...
        return self._compute_log_probs_tensor(
            generated_ids, scores_tensor, prompt_lengths
        )
...
class VERLTrainer:
    ...
    def generate_rollouts(self, prompts: List[str]) -> RolloutBatch:
        ...
        """
        # Generate responses
        responses, log_probs, prompt_len, full_seq, full_attention_mask = (
        ...
        rewards = torch.tensor(
            self.reward_model.get_rewards(
                full_texts, self.reward_model.tokenizer, self.device
            ),
            device=self.device,
            dtype=torch.float32,
        )
        ...
        if (
            hasattr(self.config.verl, "reward_clip")
            and self.config.verl.reward_clip > 0
        ):
            rewards = torch.clamp(
                rewards, -self.config.verl.reward_clip, self.config.verl.reward_clip
            )
        ...
        with torch.no_grad():
            values = self.value_model(
                input_ids=full_seq, attention_mask=full_attention_mask
            )
            values[~full_attention_mask.bool()] = 0.0

        # Compute advantages and returns using GAE
        advantages, returns = self._compute_gae(rewards, values, full_attention_mask)
    ...
    def _compute_gae(
        self, rewards: torch.Tensor, values: torch.Tensor, attention_mask: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        values = attention_mask * values

        expanded_rewards = torch.zeros_like(values)  # [batch_size, seq_len]
        last_indices = attention_mask.sum(dim=-1) - 1  # [batch_size]
        batch_indices = torch.arange(batch_size, device=values.device)
        expanded_rewards[batch_indices, last_indices] = rewards
        next_values = torch.zeros_like(values)
        next_values[:, :-1] = values[:, 1:]

        td_error = expanded_rewards + gamma * next_values - values
        ...
        for i in reversed(range(seq_len)):
            if i == seq_len - 1:
                ...
                advantages[:, i] = (
                    td_error[:, i] + gamma * gae_lambda * advantages[:, i + 1]
                )
    ...
    def _train_step_custom(self, rollout_batch: RolloutBatch) -> TrainingMetrics:
        ...
        for epoch in range(self.config.verl.ppo_epochs):
            ...
            advantages = rollout_batch.advantages[:, prompt_len - 1 : -1][
                new_log_probs_mask
            ]
            ...
            total_policy_loss_step = (
                policy_loss - self.config.training.ppo_entropy_coef * entropy
            )

            # Value function training
            values = self.value_model(
                input_ids=input_ids, attention_mask=full_attn_mask
            )
            ...
            if kl_div.item() > self.config.verl.ppo_target_kl * 2:
                logger.warning(
                    f"Early stopping due to large KL divergence: {kl_div.item()}"
                )
                break
            ...
            torch.nn.utils.clip_grad_norm_(
                self.policy.parameters(), self.config.training.ppo_max_grad_norm
            )
            ...
            value_loss.backward()
            torch.nn.utils.clip_grad_norm_(
                self.value_model.parameters(), self.config.training.ppo_max_grad_norm
            )
    ...
    def save_checkpoint(
        ...
        checkpoint = {
            ...
            "policy_optimizer_state_dict": getattr(
                self, "policy_optimizer", {}
            ).state_dict()
            if hasattr(self, "policy_optimizer")
            else {},
            "value_optimizer_state_dict": getattr(
    ...
    def load_checkpoint(self, checkpoint_path: str) -> Dict[str, Any]:
        ...
        if (
            hasattr(self, "policy_optimizer")
            and "policy_optimizer_state_dict" in checkpoint
        ):
        ...
        if (
            hasattr(self, "value_optimizer")
            and "value_optimizer_state_dict" in checkpoint
        ):
...
def create_rlhf_trainer(
    ...
    logger.info(
        f"Policy model parameters: {sum(p.numel() for p in policy_model.parameters()):,}"
    )
...
def evaluate_policy(
    trainer: VERLTrainer, eval_prompts: List[str], num_samples: int = 5
) -> Dict[str, float]:
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx in pbar:
            start_idx = batch_idx
            end_idx = min(
                start_idx + trainer.config.experiment.eval_batch_size, len(eval_prompts)
            )
            ...
            rewards = trainer.reward_model.get_rewards(
                full_texts, trainer.reward_model.tokenizer, trainer.device
            )
