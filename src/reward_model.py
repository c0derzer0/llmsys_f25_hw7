...
class RewardModelTrainer:
    ...
    def prepare_batch(self, batch: Dict) -> Dict[str, torch.Tensor]:
        ...
        return {"chosen": chosen_encoded, "rejected": rejected_encoded}
    ...
    def compute_loss(self, batch: Dict) -> torch.Tensor:
        ...
        # print("chosen_rewards", chosen_rewards.rewards.shape)
        rejected_rewards = self.model(
            input_ids=batch["rejected"]["input_ids"],
            attention_mask=batch["rejected"]["attention_mask"],
        )
        # print("rejected_rewards", rejected_rewards.rewards.shape)

        y = torch.ones_like(chosen_rewards.rewards)
        # print("y", y)
        loss = self.loss_fn(chosen_rewards.rewards, rejected_rewards.rewards, y)

        return loss, chosen_rewards.rewards, rejected_rewards.rewards
...
def create_reward_model(
    ...
    model = RewardModel(
        config=config, model_name=model_name, hidden_size=hidden_size, dropout=dropout
    )
